...
use rn_meta::models::MetadataRecord;
...
fn is_up_to_date_returns_true_when_mtime_and_size_match() {
    ...
    assert!(store
        .is_up_to_date("/tmp/current.txt", 1700000000, 1024)
        .unwrap());
}
...
fn is_up_to_date_returns_false_when_mtime_differs() {
    ...
    assert!(!store
        .is_up_to_date("/tmp/old.txt", 1700099999, 1024)
        .unwrap());
}
...
fn is_up_to_date_returns_false_when_size_differs() {
    ...
    assert!(!store
        .is_up_to_date("/tmp/resized.txt", 1700000000, 9999)
        .unwrap());
}
...
fn search_filename_prefix_finds_matching_files() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/docs/report_2024.pdf"))
        .unwrap();
    store
        .upsert_metadata(&sample_record("/docs/report_2025.pdf"))
        .unwrap();
    store
        .upsert_metadata(&sample_record("/docs/readme.md"))
        .unwrap();
...
fn search_filename_prefix_respects_limit() {
    ...
    for i in 0..10 {
        store
            .upsert_metadata(&sample_record(&format!("/docs/file_{}.txt", i)))
            .unwrap();
    }
...
fn search_filename_prefix_returns_empty_for_no_match() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/docs/hello.txt"))
        .unwrap();
...
fn set_state_updates_document_state() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/tmp/state.txt"))
        .unwrap();

    store
        .set_state("/tmp/state.txt", &DocumentState::Indexed)
        .unwrap();
...
fn set_state_records_error_for_failed_state() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/tmp/fail.txt"))
        .unwrap();
...
fn get_doc_id_returns_none_before_indexing() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/tmp/new.txt"))
        .unwrap();
...
fn get_doc_id_returns_value_after_set() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/tmp/indexed.txt"))
        .unwrap();
    store
        .set_doc_id("/tmp/indexed.txt", "tantivy-doc-42")
        .unwrap();
...
fn delete_by_path_removes_record() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/tmp/gone.txt"))
        .unwrap();
...
fn find_stale_returns_stale_documents() {
    let (store, _dir) = open_temp_store();
    store
        .upsert_metadata(&sample_record("/project/a.rs"))
        .unwrap();
    store
        .upsert_metadata(&sample_record("/project/b.rs"))
        .unwrap();

    store
        .set_state("/project/a.rs", &DocumentState::Stale)
        .unwrap();
...
fn get_stats_counts_files_by_state() {
    ...
    store
        .set_state("/s/a.txt", &DocumentState::Indexed)
        .unwrap();
    store
        .set_state("/s/b.txt", &DocumentState::Indexed)
        .unwrap();
...
fn get_failed_tasks_returns_failed_records() {
    ...
    store
        .set_state("/f/ok.txt", &DocumentState::Indexed)
        .unwrap();
    let failed = DocumentState::Failed {
        error: "timeout".into(),
        retry_count: 1,
